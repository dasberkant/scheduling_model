...
def solve_rental_scheduling_streamlit(num_orders, num_vehicles,
    ...
    # For simplicity, let all vehicles be available at time 0
    A = {v: 0 for v in V}
    ...
    #    => must do today (z[i]=0), and finish by d[i].
    for i in I:
    ...
    #    => If done today (z=0), must start by hour 24.
    for i in I:
        if 24 + p_list[i-1] <= d_list[i-1]:
            model.addConstr(
                S[i] <= 24 + big_m*z[i],
...
def main():
    ...
    then uses Gurobi to solve and display the results with Gantt charts.
    """)
    ...
    # For convenience, store the input in lists
    p_list = []
    d_list = []
    e_list = []
    l_list = []

    # Show a table-like input for each order
    ...
    if st.button("Solve"):
        try:
            ...
            # Generate Gantt charts for each vehicle
            for v in range(1, num_vehicles+1):
                ...
                for idx, (order_id, stime, duration) in enumerate(tasks):
                    ...
                    if duration < threshold:
                        ...
                        text_ha = 'center'
                        text_color = 'white'
                    ax.text(text_x, idx, label_text, ha=text_ha, va='center', color=text_color, fontsize=8)
